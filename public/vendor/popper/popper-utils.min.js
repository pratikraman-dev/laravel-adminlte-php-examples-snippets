...
function a(a, b) {
    ...
    const c = a.ownerDocument.defaultView, d = c.getComputedStyle(a, null);
    return b ? d[b] : d
}
function b(a) {
...
const f = e && !!(window.MSInputMethodContext && document.documentMode), g = e && /MSIE 10/.test(navigator.userAgent);
function h(a) {
...
function l(a, b) {
    ...
    const c = a.compareDocumentPosition(b) & Node.DOCUMENT_POSITION_FOLLOWING, d = c ? a : b, e = c ? b : a, f = document.createRange();
...
function m(a, b='top') {
    const c = 'top' === b ? 'scrollTop' : 'scrollLeft', d = a.nodeName;
    if ('BODY' === d || 'HTML' === d) {
        const b = a.ownerDocument.documentElement, d = a.ownerDocument.scrollingElement || b;
        return d[c]
    }
    return a[c]
}
function n(a, b, c=!1) {
    const d = m(b, 'top'), e = m(b, 'left'), f = c ? -1 : 1;
    return a.top += d * f, a.bottom += d * f, a.left += e * f, a.right += e * f, a
}
function o(a, b) {
    const c = 'x' === b ? 'Left' : 'Top', d = 'Left' == c ? 'Right' : 'Bottom';
    return parseFloat(a[`border$
    {
        c
    }
    Width`],
...
function p(a, b, c, d) {
    ...
    +parseInt(d[`margin$
    {
        'Height' === a ? 'Bottom' : 'Right'
    }`]):
    0
)
}
function q(a) {
    const b = a.body, c = a.documentElement, d = h(10) && getComputedStyle(c);
...
function t(b) {
    ...
    }, e = 'HTML' === b.nodeName ? q(b.ownerDocument) : {}, f = e.width || b.clientWidth || d.width, g = e.height || b.clientHeight || d.height;
    let i = b.offsetWidth - f, j = b.offsetHeight - g;
    if (i || j) {
        const c = a(b);
...
function u(b, d, e=!1) {
    var f = Math.max;
    const g = h(10), i = 'HTML' === d.nodeName, j = t(b), k = t(d), l = c(b), m = a(d), o = parseFloat(m.borderTopWidth, 10), p = parseFloat(m.borderLeftWidth, 10);
    ...
    if (q.marginTop = 0, q.marginLeft = 0, !g && i) {
        const a = parseFloat(m.marginTop, 10), b = parseFloat(m.marginLeft, 10);
...
function v(a, b=!1) {
    var c = Math.max;
    const d = a.ownerDocument.documentElement, e = u(a, d), f = c(d.clientWidth, window.innerWidth || 0), g = c(d.clientHeight, window.innerHeight || 0), h = b ? 0 : m(d), i = b ? 0 : m(d, 'left'), j = {
...
function y(a, e, f, g, h=!1) {
    ...
    if ('viewport' === g)i = v(j, h); else {
        let d;
        'scrollParent' === g ? (d = c(b(e)), 'BODY' === d.nodeName && (d = a.ownerDocument.documentElement)) : 'window' === g ? d = a.ownerDocument.documentElement : d = g;
        const f = u(d, j, h);
        if ('HTML' === d.nodeName && !w(j)) {
            const {height:b,width:c}=q(a.ownerDocument);
            i.top += f.top - f.marginTop, i.bottom = b + f.top, i.left += f.left - f.marginLeft, i.right = c + f.left
        } else i = f
    }
    f = f || 0;
...
function A(a, b, c, d, e, f=0) {
    ...
    }, i = Object.keys(h).map((a) = > r({key: a}, h[a], {area: z(h[a])})
).
...
>
    {
        b || (b = !0, window.Promise.resolve().then(() = > {b = !1, a()
    }
))
}
}
...
function J(a) {
    const b = a.ownerDocument.defaultView, c = b.getComputedStyle(a), d = parseFloat(c.marginTop || 0) + parseFloat(c.marginBottom || 0), e = parseFloat(c.marginLeft || 0) + parseFloat(c.marginRight || 0), f = {
...
function N(a) {
    const b = [!1, 'ms', 'Webkit', 'Moz', 'O'], c = a.charAt(0).toUpperCase() + a.slice(1);
...
function T(a, b) {
    return S(a).removeEventListener('resize', b.updateBound), b.scrollParents.forEach((a) = > {a.removeEventListener('scroll', b.updateBound)
}
),
...
function U(a, b, c) {
    ...
    return d.forEach((a) = > {a['function'] && console.warn('`modifier.function` is deprecated, use `modifier.fn`!');
...
function W(a, b) {
    Object.keys(b).forEach((c) = > {let d = '';
    -1 !== ['width', 'height', 'top', 'right', 'bottom', 'left'].indexOf(c) && R(b[c]) && (d = 'px'), a.style[c] = b[c] + d
}
)}
function X(a, b, d, e) {
    const f = 'BODY' === a.nodeName, g = f ? a.ownerDocument.defaultView : a;
